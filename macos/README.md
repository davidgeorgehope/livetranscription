...
- **Prep docs per call**: drop `md/txt/pdf/docx` on the window (or `+ Add`). They outrank every other source for that call and are archived with its transcript. Anything written to `~/Library/Application Support/Cue/prep/inbox/` is attached the moment it lands. With `GROK_BOT_HOOK_URL`/`GROK_BOT_HOOK_KEY` in `.env`, the **Grok Bot brief** button fires your webhook-triggered automation with the call topic; the bot researches your threads and writes a one-page brief into that inbox via its local computer tool. Do it a few minutes before the call — the run is async and takes a while. **Listen also fires this automatically** for the meeting today's calendar says you're in (no calendar → whatever meeting is in progress), silently; the brief attaches mid-call when it lands and is used from the next pass on. Skipped if that brief is already pending or a brief arrived in the last 30 min.
- **Day calendar**: with `GROK_BOT_CALENDAR_URL`/`GROK_BOT_CALENDAR_KEY` in `.env`, Cue asks a third automation for today's meetings — at launch, at Listen, and when a meeting app takes the mic, if the last read is missing or over 2 h old. The bot writes `~/Library/Application Support/Cue/calendar/<yyyy-MM-dd>.json`; Cue uses it to know which meeting a call is (wrap title, Listen brief) and where it ends.
- **Call boundaries**: Cue watches which app holds the microphone (CoreAudio process objects — Zoom, Teams, Chrome for Meet; no permissions, no per-app code). Idle + an app takes the mic during a meeting on today's calendar that Cue hasn't listened to → Listen starts on its own (Settings toggle; Stop or Not now skips that meeting); otherwise the "on a call? Listen" banner. Listening + the app lets go for 15 s → 30 s cancellable countdown, then Stop and the wrap. If the app takes the mic back once the calendar has moved on (the meeting's scheduled end has passed, or the next one starts within 5 min), that's the next meeting: Cue saves the call — wrap and hand-off as on Stop — and starts a new one; earlier than that it's a rejoin. The schedule is today's calendar, else the brief's `meeting_start`/`meeting_end` lines (or its human `When:` line, via Apple's date detector); past the scheduled end, 90 s of silence also stops. Pure silence with neither signal only *suggests* stopping. `scripts/repro_day_calendar.swift` checks these rules.
